...
const ease1 = [0.22, 1, 0.36, 1]
const t06 = { duration: 0.6, ease: ease1 }
const t08 = { duration: 0.8, ease: ease1 }
const t10 = { duration: 1.0, ease: ease1 }
const t14 = { duration: 1.4, ease: ease1 }
const tProgress = { duration: 0.1 }
...
const progressStyle = { transformOrigin: '0%', position: 'fixed', top: 0, left: 0, right: 0, height: '3px', background: '#8B1E2D', zIndex: 50 } as const
...
function Hero() {
  ...
  const bgStyle = { y: yBg, opacity: opacityBg, backgroundImage: 'url(' + photo('BUENA PAREJA.jpg') + ')' }
  return (
    <section ref={ref} className="relative h-[100svh] min-h-[700px] w-full overflow-hidden">
      ...
      <div className="relative z-10 h-full w-full flex flex-col items-center justify-center text-center px-6">
        ...
        <motion.div initial={fadeIn} animate={showIn} transition= duration: 1.6, delay: 0.6, ease: ease1  className="mt-10 max-w-2xl">
        ...
        <motion.div initial={fadeIn} animate={showIn} transition= duration: 2, delay: 1.4  className="absolute bottom-10 left-1/2 -translate-x-1/2">
  ...
  return (
    <section className="relative py-28 md:py-40 px-6 bg-crema grain">
      <div className="max-w-3xl mx-auto text-center">
        ...
        <motion.div initial={fadeUp} whileInView={show} viewport={vpOnce} transition= duration: 1, ease: ease1, delay: 0.15 >
        ...
        <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition= duration: 1, delay: 0.4, ease: ease1  className="hand text-2xl md:text-3xl text-rojo mt-12">
...
function FacetasIntro() {
  return (
    <section className="py-20 md:py-28 px-6 bg-cremaClara text-center">
      ...
      <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition= duration: 0.8, delay: 0.2, ease: ease1  className="serif italic text-marronSuave text-lg md:text-xl mt-6 max-w-xl mx-auto">
...
function FacetaCard({ a, idx }: { a: Aspecto; idx: number }) {
  ...
  return (
    <article className={'grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center py-16 md:py-24 ' + (right ? '' : '')}>
      ...
      <motion.div initial={txtInit} whileInView={slideShow} viewport={vpMore} transition= ...t10, delay: 0.1  className={right ? 'md:order-1' : ''}>
...
function GaleriaItemCard({ g, i }: { g: GaleriaItem; i: number }) {
  const rot = (i % 5 === 0 ? -2 : i % 5 === 1 ? 1.5 : i % 5 === 2 ? -1 : i % 5 === 3 ? 2 : 0)
  const rotStyle = { transform: 'rotate(' + rot + 'deg)' }
  return (
    <motion.div initial={scaleIn} whileInView={scaleShow} viewport={vpMore} transition= ...t08, delay: (i % 8) * 0.05  style={rotStyle} className="bg-cremaClara p-3 pb-12 img-frame">
  ...
  return (
    <section className="py-28 md:py-36 px-6 bg-crema">
      <div className="max-w-4xl mx-auto">
        ...
        <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition= ...t08, delay: 0.1  className="serif italic text-marronSuave text-center text-lg max-w-2xl mx-auto">
          Diez canciones que cuentan nuestra historia, en orden.
        </motion.p>
        <Divider />
        <ol className="space-y-3 md:space-y-4">
          {SONGS.map((s, i) => (
            <motion.li key={s.n} initial={slideR} whileInView={slideShow} viewport={vpMore} transition= ...t06, delay: i * 0.04  className="flex items-center gap-5 p-4 md:p-5 bg-cremaClara border border-dorado/20 hover:border-rojo/30 transition-colors">
  ...
  return (
    <section className="relative py-28 md:py-36 px-6 bg-marron text-crema overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto">
        ...
        <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition= ...t08, delay: 0.2  className="serif italic text-center text-crema/80 text-xl mt-6 max-w-2xl mx-auto">
          Yo hice nuestro propio video con esta canción. Cuando lo tengas listo, vívelo aquí. Mientras tanto, escúchala.
        </motion.p>
        <motion.div initial={fadeUp} whileInView={show} viewport={vpOnce} transition= ...t08, delay: 0.3  className="mt-12 max-w-2xl mx-auto">
          <audio controls className="w-full" preload="metadata">
            <source src={AUDIO_18} type="audio/mpeg" />
          </audio>
        </motion.div>
        <motion.div initial={fadeUp} whileInView={show} viewport={vpOnce} transition= ...t08, delay: 0.4  className="mt-12 max-w-3xl mx-auto">
        ...
        <div className="mt-20 space-y-12">
          {VIDEO_18.map((v, i) => (
            <motion.div key={i} initial={fadeUp} whileInView={show} viewport={vpMore} transition= ...t10, delay: 0.05 * (i % 4)  className={'grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center ' + (i % 2 === 1 ? 'md:[direction:rtl]' : '')}>
              <div className="aspect-[4/5] overflow-hidden img-frame md:[direction:ltr]">
                <img src={photo(v.foto)} alt={v.caption} loading="lazy" className="w-full h-full object-cover" />
              </div>
              <p className="dance text-3xl md:text-5xl text-doradoSuave leading-snug md:[direction:ltr]">
                {v.caption}
              </p>
            </motion.div>
          ))}
...
function TimelineItem({ item, i }: { item: { hora: string; titulo: string; texto: string }; i: number }) {
  return (
    <motion.div initial={slideR} whileInView={slideShow} viewport={vpMore} transition= ...t08, delay: i * 0.05  className="relative pl-10 pb-10 border-l border-dorado/40 last:border-l-transparent">
  ...
  return (
    <section className="py-28 md:py-36 px-6 bg-cremaClara">
      <div className="max-w-3xl mx-auto">
        ...
        <div>
          {VIERNES.map((v, i) => <TimelineItem key={i} item={v} i={i} />)}
        </div>
        <h3 className="hand text-rojo text-3xl mt-12 mb-8">Sábado 23</h3>
        <div>
          {SABADO.map((v, i) => <TimelineItem key={i} item={v} i={i} />)}
        </div>
      </div>
    </section>
  )
  ...
  return (
    <section className="py-28 md:py-36 px-6 bg-crema">
      <div className="max-w-5xl mx-auto">
        ...
        <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition= ...t08, delay: 0.1  className="serif italic text-marronSuave text-center text-lg mt-6 max-w-2xl mx-auto">
          Tu lenguaje de amor es tiempo de calidad. Tu regalo es estar.
        </motion.p>
        <Divider />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {PLAN_JUNTOS.map((p, i) => (
            <motion.div key={i} initial={fadeUp} whileInView={show} viewport={vpMore} transition= ...t08, delay: i * 0.06  className="bg-cremaClara border border-dorado/20 p-8 hover:border-rojo/30 transition-colors">
  ...
  return (
    <section className="py-28 md:py-40 px-6 bg-rojo text-crema relative overflow-hidden">
      <div className="absolute inset-0 opacity-10">
        <div className="absolute inset-0" style= backgroundImage: 'radial-gradient(#F4EEE3 1px, transparent 1px)', backgroundSize: '40px 40px'  />
      </div>
      <div className="relative max-w-3xl mx-auto text-center">
        ...
        <motion.p initial={scaleIn} whileInView={scaleShow} viewport={vpOnce} transition= ...t14, delay: 0.3  className="script text-doradoSuave text-6xl md:text-8xl mt-16 leading-tight">
          Y yo te hallé a ti.
        </motion.p>
        <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition= ...t08, delay: 0.5  className="serif italic text-crema/80 text-lg md:text-xl mt-12 max-w-2xl mx-auto">
  ...
  return (
    <section className="relative min-h-[100svh] py-28 md:py-36 px-6 bg-crema overflow-hidden">
      <div className="max-w-3xl mx-auto text-center">
        ...
        <motion.h2 initial={fadeUp} whileInView={show} viewport={vpOnce} transition= ...t10, delay: 0.2  className="serif text-5xl md:text-7xl text-rojo mt-12 leading-tight">
          Feliz cumpleaños,<br/><span className="italic">mi amor.</span>
        </motion.h2>
        <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition= ...t10, delay: 0.4  className="serif italic text-marronSuave text-xl md:text-2xl mt-8 max-w-xl mx-auto">
          Gracias por dejarme amarte. Gracias por dejarte amar. Que este año Dios te corone con lo mejor.
        </motion.p>
        <motion.div initial={fadeUp} whileInView={show} viewport={vpOnce} transition= ...t10, delay: 0.6  className="mt-16">
