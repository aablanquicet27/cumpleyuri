...
// Common transitions (NEW object each render via fn so framer-motion no muta refs compartidas)
const t06 = mkT(0.6)
const t08 = mkT(0.8)
const t10 = mkT(1.0)
const t14 = mkT(1.4)
...
// Patron de fondo crema
const patternStyle = { backgroundImage: 'radial-gradient(#F4EEE3 1px, transparent 1px)', backgroundSize: '20px 20px' }

function ScrollProgress() {
  const sp = useScroll()
  const sx: MotionValue<number> = useTransform(sp.scrollYProgress, [0, 1], [0, 1])
  // MotionValue debe ir DENTRO de style, no en animate
  const style = {
    transformOrigin: '0%',
  ...
  return (
    <section ref={ref} className="relative h-[100svh] min-h-[700px] w-full overflow-hidden">
      ...
      <div className="relative z-10 h-full w-full flex flex-col items-center justify-center text-center px-6">
        <motion.p initial={fadeIn} animate={showIn} transition={mkT(1.4)} className="hand text-crema text-2xl md:text-3xl mb-6 tracking-wide">22 de mayo de 2026</motion.p>
        <motion.h1 initial={fadeUp} animate={show} transition={mkT(1.4)} className="serif text-crema text-5xl md:text-7xl lg:text-8xl leading-[1.05] tracking-tightish max-w-4xl">
          Feliz Cumpleaños,<br/><span className="italic font-light">la mejor mujer del planeta</span>
        </motion.h1>
        <motion.div initial={fadeIn} animate={showIn} transition={mkT(1.4, 0.6)} className="mt-10 max-w-2xl">
...
function FacetasIntro() {
  return (
    <section className="py-20 md:py-28 px-6 bg-cremaClara text-center">
      ...
      <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition={mkT(0.8, 0.2)} className="serif italic text-marronSuave text-lg md:text-xl mt-6 max-w-xl mx-auto">
...
function FacetaCard({ a, idx }: { a: Aspecto; idx: number }) {
  ...
  return (
    <article className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center py-16 md:py-24">
      ...
      <motion.div initial={txtInit} whileInView={slideShow} viewport={vpMore} transition={mkT(1.0, 0.15)} className={right ? 'md:order-1' : ''}>
...
function GaleriaItemCard({ g, i }: { g: GaleriaItem; i: number }) {
  ...
  return (
    <motion.div initial={scaleIn} whileInView={scaleShow} viewport={vpMore} transition={mkT(0.6, i * 0.04)} style={rotStyle} className="bg-cremaClara p-3 pb-12 img-frame">
...
function SongRow({ s, i }: { s: { n: number; t: string; sub: string }; i: number }) {
  return (
    <motion.li initial={slideR} whileInView={slideShow} viewport={vpMore} transition={mkT(0.6, i * 0.05)} className="flex items-center gap-5 p-4 md:p-5 bg-cremaClara border border-dorado/20 hover:border-rojo/30 transition-colors">
  ...
  return (
    <section className="relative py-28 md:py-36 px-6 bg-marron text-crema overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto">
        ...
        <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition={mkT(0.6, 0.2)} className="serif italic text-center text-crema/80 text-xl mt-6 max-w-2xl mx-auto">
...
function TimelineItem({ item, i }: { item: { hora: string; titulo: string; texto: string }; i: number }) {
  return (
    <motion.div initial={slideR} whileInView={slideShow} viewport={vpMore} transition={mkT(0.6, i * 0.06)} className="relative pl-10 pb-10 border-l border-dorado/40 last:border-l-transparent">
...
function PlanCard({ p, i }: { p: { emoji: string; titulo: string; texto: string }; i: number }) {
  return (
    <motion.div initial={fadeUp} whileInView={show} viewport={vpMore} transition={mkT(0.6, i * 0.06)} className="bg-cremaClara border border-dorado/20 p-8 hover:border-rojo/30 transition-colors">
  ...
  return (
    <section className="py-28 md:py-36 px-6 bg-crema">
      <div className="max-w-5xl mx-auto">
        ...
        <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition={mkT(0.8, 0.2)} className="serif italic text-marronSuave text-center text-lg mt-6 max-w-2xl mx-auto">
  ...
  return (
    <section className="py-28 md:py-40 px-6 bg-rojo text-crema relative overflow-hidden">
      ...
      <div className="relative max-w-3xl mx-auto text-center">
        ...
        <motion.p initial={scaleIn} whileInView={scaleShow} viewport={vpOnce} transition={mkT(1.4, 0.3)} className="script text-doradoSuave text-6xl md:text-8xl mt-16 leading-tight">
          Y yo te hallé a ti.
        </motion.p>
        <motion.p initial={fadeUp} whileInView={show} viewport={vpOnce} transition={mkT(0.8, 0.6)} className="serif italic text-crema/80 text-lg md:text-xl mt-12 max-w-2xl mx-auto">
  ...
  return (
    <section className="relative min-h-[100svh] py-28 md:py-36 px-6 bg-crema overflow-hidden">
      <div className="max-w-3xl mx-auto text-center">
        ...
        <motion.h2 initial={fadeUp} whileInView={show} viewport={vpOnce} transition={mkT(1.0, 0.2)} className="serif text-5xl md:text-7xl text-rojo mt-12 leading-tight">
